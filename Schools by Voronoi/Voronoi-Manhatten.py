...
def get_entrance(point, num, streets):
    ...
    if pos == len(spec_streets) - 1:
        ...
        if spec_streets[pos+1] == point[num]:
            ...
            if min(abs(float(spec_streets[pos])-float(spec_streets[pos+1])),
...
def visualize(massive_of_lines, color):
    for point in massive_of_lines:
...
visualize(schools, 'red')
visualize(coffee_shops, 'brown')
visualize(houses, 'blue')
plt.show()
